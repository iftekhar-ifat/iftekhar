...
- **Encoder:** Instead of producing a single point in the latent space, it outputs two things: a mean $$\mu$$ and a standard deviation $$\sigma$$. These define a probability distribution (usually a Gaussian) for each input image.

- **Sampling:** We then sample a latent vector $$z$$ from this distribution using a clever trick called *reparameterization* (more on that in a moment).

- **Decoder:** Just like before, the decoder takes this $$z$$ and reconstructs the image or even generates a new one if we sample $$z$$ ourselves!
...
Since our autoencoder used convolutional layers, we’ll build a convolutional VAE for consistency. The encoder will output both $$\mu$$ and $$\log(\sigma^2)$$ (log-variance, which is more stable to compute), and we’ll sample $$z$$ before passing it to the decoder. Here’s the code:
...
- **Encoder:** Similar to our autoencoder, but after flattening, we split into two linear layers: one for $$\mu$$ and one for $$\log(\sigma^2)$$. We set `latent_dim=16` to keep the latent space manageable.
- **Reparameterization Trick:** This is the magic that lets us train with sampling. Instead of sampling directly (which isn’t differentiable), we compute $$z = \mu + \sigma \cdot \epsilon$$, where $$\epsilon$$ is random noise from a standard normal distribution. This keeps the gradient flow intact.
- **Decoder:** Almost identical to our autoencoder’s decoder, turning the sampled $$z$$ back into an image.
...
- **KL Divergence:** A mathematical term that regularizes the latent space, keeping $$\mu$$ near 0 and $$\sigma$$ near 1.
